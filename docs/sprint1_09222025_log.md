...
1.  **Implemented `/garmin/health` Endpoint:**
    *   Added a new Flask route `/garmin/health` to `garmin_api.py` to retrieve daily health summaries.
    *   Integrated `api_client.get_user_summary(cdate)` to fetch data from Garmin Connect.
    *   Included basic date handling, defaulting to today's date if `cdate` is not provided.

2.  **Updated Test Suite (`test_garmin_api.py`):**
    *   Added a mock `get_user_summary` method to the `MockGarmin` class to simulate health data retrieval.
    *   Introduced new test cases: `test_garmin_health_summary_success` and `test_garmin_health_summary_not_logged_in` to verify the functionality and error handling of the new `/garmin/health` endpoint.

3.  **Refactored with `garmin_api_wrapper` Decorator:**
    *   Created `python-scripts/garmin_utils.py` containing a `garmin_api_wrapper` decorator. This decorator centralizes common logic for checking `api_client` availability and handling `try...except` blocks for consistent error responses.
    *   Applied the `@garmin_api_wrapper` decorator to the `/garmin/activities`, `/garmin/activity_detail`, `/garmin/activity_download`, and `/garmin/health` endpoints in `garmin_api.py`.
    *   **Resolved Circular Import Issue:** Diagnosed and fixed a circular import between `garmin_api.py` and `garmin_utils.py` by modifying the decorator to access `api_client` from the global scope of the decorated function's module (`f.__globals__.get('api_client')`).

4.  **Verification:**
    *   Successfully rebuilt Docker containers and ran all unit tests. All tests passed, confirming no regressions were introduced by the refactoring and new feature implementation.

5.  **Commit:**
    *   Committed the implementation of the `/garmin/health` endpoint and the refactoring changes to centralize error handling and login checks.
